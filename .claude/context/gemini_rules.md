...
*   **Spatial Footprint (MOC) Placement:** Any entity with a real spatial footprint (not just a point) must carry a MOC-representable field — the `footprint` column, a native PostgreSQL `INT8MULTIRANGE` MOC representation backed by `src/db/spatial_types.MOCType` (landed in doc 27, replacing the earlier Text `moc_str` column and the never-viable `healpix-alchemy` dependency; `Tile.healpix_index` migrating to a native range type remains open per issue #26). For an entity with a raw/derived split — an immutable ingest record vs. our processed reference to it — the footprint field belongs on the derived side, both to protect the raw record's immutability (footprint computation can be revisited; the raw ingest shouldn't be) and because the derived entity is what actually participates in downstream spatial joins/associations.
*   **Backlog-Sourced Revisions:** A revision prompt item may sometimes originate from Claude's `assessPrompt` cross-referencing the open tech-debt/enhancement backlog against a draft's scope, rather than from a gap in the draft itself. These are explicitly labeled in the prompt (e.g., "per backlog issue #N"). Treat these as intentional, pre-existing concerns being folded in now — not as feedback on drafting quality — since the change may otherwise look discontinuous with the rest of a normal refinement pass.

## Logs
###### 2026-07-27 — Spatial Footprint principle updated for the native MOC column
Updated the Spatial Footprint (MOC) Placement principle to reflect doc 27's implementation: the footprint field is now the `footprint` column, a native PostgreSQL `INT8MULTIRANGE` backed by `src/db/spatial_types.MOCType`, replacing the earlier Text `moc_str` column and the `healpix-alchemy` dependency (which was unmaintained and unimportable against modern `mocpy`). The raw/derived placement guidance is unchanged. `Tile.healpix_index`'s migration to a native range type remains open per issue #26.

###### 2026-07-22 — Backlog cross-reference added to assessPrompt
Added a Backlog Cross-Reference step to `assess_arch.md`: before running the §A–§G checklist, Claude now does a best-effort pass over open `tech-debt`/`enhancement` GitHub Issues to flag any that touch what the doc under review is actually creating or modifying, surfacing them as Decision Required items rather than folding them in silently. Explicitly scoped as best-effort, not exhaustive, since the backlog grows monotonically and the user separately reviews it directly as a backstop. Added the corresponding Backlog-Sourced Revisions guiding principle here so Gemini has context when a revision prompt item traces back to a pre-existing backlog issue instead of a gap in its own draft.
