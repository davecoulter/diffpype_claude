...
**Backlog cross-reference:** Before running the checklist, fetch every open GitHub Issue labeled `tech-debt` or `enhancement` (via `gh issue list`). Cross-reference each against this doc's actual scope — the specific models, fields, or behaviors it creates or modifies right now, not just thematically related work. Flag an issue only if it touches something the doc is materially creating or changing in this pass. This is a best-effort pass over the currently open backlog, not an exhaustive audit — the user separately reviews the backlog directly as a backstop, so don't over-invest in this check as the backlog grows.
...
2. **Backlog Cross-Reference** — list every open `tech-debt`/`enhancement` issue relevant to what this doc creates or modifies, one line each: issue number, title, and a one-sentence reason naming the specific field/model/behavior it touches. If none apply, state that explicitly with a one-line reason. Relevant issues are never folded in automatically — flag each as **Decision Required** and wait for confirmation. If confirmed, label the corresponding item in the consolidated Gemini prompt as sourced from the backlog (e.g., "per backlog issue #N") so Gemini understands it's a pre-existing tracked concern being folded in now, not a gap found in its draft.
3. **"What fixed cleanly" bullet list** — on re-assessment passes, briefly note which prior failing items are now resolved, before detailing anything still open.
