...
void vargs(int argc, char *argv[]) {
   ...
   sprintf(help, "%s   [-gridssc 0|1]     : move in a grid if exact position in -ssf|-cmp file did not work (%d)\n", help, gridSSC);
   ...
   for (iarg=1; iarg < argc; iarg++) {
      if (argv[iarg][0]=='-') {
         ...
         if (strcasecmp(argv[iarg]+1,"inim")==0) {
            ...
            sstampFile = argv[++iarg];
	    cmpFile = 1;
         } else if (strcasecmp(argv[iarg]+1,"gridssc")==0) {
            sscanf(argv[++iarg], "%d", &gridSSC);
