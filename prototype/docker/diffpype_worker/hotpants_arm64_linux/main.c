...
int main(int argc,char *argv[]) {
  int         i,j,k,l,m,ii;                          /* generic indices */
   ...
   */
   ...
   if (useFullSS) {
      ...
      fwXStamp     = fwKernel;
      fwYStamp     = fwKernel;
      nStampX     = (int)(imin(tNaxes[0], iNaxes[0]) / nRegX / fwXStamp);
      nStampY     = (int)(imin(tNaxes[1], iNaxes[1]) / nRegY / fwYStamp);
      ...
      fwXStamp = imin(tNaxes[0], iNaxes[0]) / nRegX / nStampX;
      fwYStamp = imin(tNaxes[1], iNaxes[1]) / nRegY / nStampY;

      /* fwStamp -= fwKernel; */
      fwXStamp -= fwXStamp % 2 == 0 ? 1 : 0; /* hmmm, an odd shape... */
      fwYStamp -= fwYStamp % 2 == 0 ? 1 : 0; /* hmmm, an odd shape... */

      /* insanity checking */
      if (fwXStamp < fwKSStamp) {
	 fwXStamp  = fwKSStamp+fwKernel;
	 fwXStamp -= fwYStamp % 2 == 0 ? 1 : 0;

	 nStampX = imin(tNaxes[0], iNaxes[0]) / nRegX / fwXStamp;
	 fprintf(stderr, "WARNING : too many stamps requested\n");
	 fprintf(stderr, "          using nsx = %d\n", nStampX);
      }
      if (fwYStamp < fwKSStamp) {
	 fwYStamp  = fwKSStamp+fwKernel;
	 fwYStamp -= fwYStamp % 2 == 0 ? 1 : 0;

	 nStampY = imin(tNaxes[1], iNaxes[1]) / nRegY / fwYStamp;
	 fprintf(stderr, "WARNING : too many stamps requested\n");
	 fprintf(stderr, "          using nsy = %d\n", nStampY);
      }
   ...
   for (i = 0; i < nR; i++) {
	 ...
	 NskippedSubstamps = 0;
      ...
      if (nR > 1) {
         ...
         rXBMin = imax(xMin, rXMin - fwXStamp/2);
         rYBMin = imax(yMin, rYMin - fwYStamp/2);
         rXBMax = imin(xMax, rXMax + fwXStamp/2);
         rYBMax = imin(yMax, rYMax + fwYStamp/2);
      }
      else {
      ...
      if (!(kernelImIn)) {
	   ...
	   if (verbose>=2) fprintf(stderr,"Allocating image stamps...\n");
	   ...
	   /*if (!(strncmp(forceConvolve, "t", 1)==0)) {*/
	    if (verbose>=2) fprintf(stderr,"Allocating template stamps...\n");
	...
	mask out bad input pixels, spread by hwKernel * kfSpreadMask1
	 ...
	 while ((status <= 2) && (flag)) {
	    ...
	    if (verbose >= 2) fprintf(stderr, "region size: (X,Y)=(%d,%d)\n",rPixX,rPixY );
	    if (verbose >= 2) fprintf(stderr, "# stamps: (X,Y)=(%d,%d)\n", nStampX,nStampY);
	    if (verbose >= 2) fprintf(stderr, "stamp size: (X,Y)=(%d,%d)\n",fwXStamp,fwYStamp );
	    ...
	    for (l = 0; l < nStampY; l++) {
		  ...
		  sXMax = imin(sXMin + fwXStamp - 1, rXBMax);
		  sYMax = imin(sYMin + fwYStamp - 1, rYBMax);
		  ...
		  if (sstampFile) {
		    if (verbose >= 2) fprintf(stderr, "\nAdding centers manually to s(%d,%d) x:%d-%d y:%d-%d\n",k,l,sXMin,sXMax,sYMin,sYMax);
		    if (verbose >= 2) fprintf(stderr, "X stamp: %d-%d region:%d-%d\n",sXMin,sXMax,rXBMin,rXBMax);
		    if (verbose >= 2) fprintf(stderr, "Y stamp: %d-%d region:%d-%d\n",sYMin,sYMax,rYBMin,rYBMax);
		     for (m = 0; m < Ncmp; m++) {
		       /*fprintf(stderr, "XXXXXXXXX: %d %d %f %f \n",hwKernel,hwKSStamp,xcmp[m],ycmp[m]);*/
		       /*if ((xcmp[m] > sXMin + hwKernel + 1) && (xcmp[m] < sXMax - hwKernel - 1) &&
			 (ycmp[m] > sYMin + hwKernel + 1) && (ycmp[m] < sYMax - hwKernel - 1)) {*/
		       /*if (verbose >= 2) fprintf(stderr, "%3d: %7.2f %7.2f\n",m,xcmp[m],ycmp[m]);*/
		       nss_t=ctStamps[ntS].nss;
		       nss_i=ctStamps[niS].nss;
		       if ((xcmp[m] > sXMin  + 1) && (xcmp[m] < sXMax  - 1) &&
			   (xcmp[m] > rXBMin + hwKernel + 1) && (xcmp[m] < rXBMax - hwKernel - 1) &&
			   (ycmp[m] > sYMin  + 1) && (ycmp[m] < sYMax  - 1) &&
			   (ycmp[m] > rYBMin + hwKernel + 1) && (ycmp[m] < rYBMax - hwKernel - 1) 
			   ) {
			 if (verbose >= 3) fprintf(stderr, "YES!!!!!!!!!!!!!!!!!!!!!!!! %3d: %7.2f %7.2f\n",m,xcmp[m],ycmp[m]);
			   buildStamps(sXMin, sXMax, sYMin, sYMax, &niS, &ntS, 0, rXBMin, rYBMin,
				       ciStamps, ctStamps, iRData, tRData,
				       xcmp[m] - rXBMin, ycmp[m] - rYBMin);

			}
		       if (gridSSC && (ctStamps[ntS].nss==nss_t) && (ctStamps[niS].nss==nss_i)){
			 for(ii=1;ii<hwKSStamp-hwKernel;ii++){
			   for (pm_x=-1;pm_x<2;pm_x+=2){
			     for (pm_y=-1;pm_y<2;pm_y+=2){
			       xx = xcmp[m] + pm_x*ii;
			       yy = ycmp[m] + pm_y*ii;
			       /*fprintf(stderr, "XXXXXXXXX: %.2f %.2f\n",xx,yy);*/
			       
			       if ((xx > sXMin  + 1) && (xx < sXMax  - 1) &&
				   (xx > rXBMin + 1) && (xx < rXBMax - 1) &&
				   (yy > sYMin  + 1) && (yy < sYMax  - 1) &&
				   (yy > rYBMin + 1) && (yy < rYBMax - 1) &&
				   (ctStamps[ntS].nss==nss_t) && (ctStamps[niS].nss==nss_i)
				   ) {
				 buildStamps(sXMin, sXMax, sYMin, sYMax, &niS, &ntS, 0, rXBMin, rYBMin,
					     ciStamps, ctStamps, iRData, tRData,
					     xx - rXBMin, yy - rYBMin);
   ...
   if (kernelImOut) {
	 ...
	 printError(status);
      ...
      /* in diffim file */
      fits_write_key_log(oPtr, "KERINFO", 0, "", &status);
      fits_write_key_longstr(oPtr, "KERFILE", kernelImOut, "", &status);
      /* in kernel */
